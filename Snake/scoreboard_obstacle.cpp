...
scoreboard_obstacle::scoreboard_obstacle(QWidget *parent) : QWidget(parent)
{
    setWindowTitle("Scoreboard with obstacle in game");
    ...
    //Background image
    background = QPixmap("/home/krzysiek89/Desktop/QT_aplikacje/The_snake_game/scoreboard_photo1.png").scaled(x, y,Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    ...
    connect(menu_button, &QPushButton::clicked, this, &scoreboard_obstacle::menuApp);
    ...
    connect(exit_button, &QPushButton::clicked, this, &scoreboard_obstacle::exitApp);
    ...
    connect(reset_button, &QPushButton::clicked, this, &scoreboard_obstacle::resetRanking);
...
void scoreboard_obstacle::paintEvent(QPaintEvent *event) {
    QWidget::paintEvent(event);
    QPainter painter(this);
    painter.drawPixmap(0, 0, background);//first draw background, later added elements on the background
}

void scoreboard_obstacle::setPlayerName(const QString &name) {
    playerName = name;
}

void scoreboard_obstacle::exitApp()
{
    this->close();//turn off app...
}

void scoreboard_obstacle::menuApp()
{
    this->close();
...
void scoreboard_obstacle::loadScores() {//based on game with obstacles
    QFile file("scores_obstacles.txt");//from QFile named scores.txt
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {//open in read mode based on text
        ...
        while (!in.atEnd()) {//while not end
            ...
            if (parts.size() == 2) {//if nickname and score exist
                ...
                qDebug() << "Invalid line format in scores_obstacles.txt:" << line;
            }
        }
        file.close();
    } else {
        qDebug() << "Failed to open scores_obstacles.txt for reading!";
    }
...
void scoreboard_obstacle::txt_show()
{
...
void scoreboard_obstacle::resetRanking()
{
    ...
    QFile file("scores_obstacles.txt");
